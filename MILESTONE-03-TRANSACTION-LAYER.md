...
3C 先实现经典 RFC 3261 状态；RFC 6026 Accepted 修正在 3D 增加：

```text
Calling ─1xx→ Proceeding
Calling/Proceeding ─2xx→ Terminated
...
下面的组件图展示 Transport、Transaction 路由、单个 Transaction、Timer 和 TU 之间的关系。NICT/NIST 与 Non-INVITE Dispatcher 已在 3B 实现，ICT/IST 与 INVITE Dispatcher 已在 3C 实现。
...
flowchart TB
    ...
    HANDLER -. "InboundSipMessage" .-> INVITE_DISPATCHER
...
当前 3A 已提供 Header、Key、Repository、Mailbox、Timer 和 TU Callback 基础设施；3B 已完成 Non-INVITE 事务；3C 已在同一套事件和并发边界上完成经典 RFC 3261 ICT/IST、非 2xx ACK 以及 UDP INVITE 闭环。下一步 3D 增加 RFC 6026 Accepted、Timer M/L、重复 2xx、CANCEL 关联和统一收尾。
