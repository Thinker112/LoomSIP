...
- `InviteTransactionManager` 分离维护 ICT/IST Repository，并负责 INVITE、响应和 ACK 路由。
- ICT 已实现 Calling、Proceeding、Completed、Terminated 以及 Timer A、B、D。
- IST 已实现 Proceeding、Completed、Confirmed、Terminated 以及 Timer G、H、I。
- 300-699 响应自动生成事务层 ACK；重复最终响应触发 ACK 重发。
- 重复 INVITE 不重复通知 TU，并按状态重发最近响应。
- 匹配非 2xx ACK 首次通知 TU，Confirmed 中的重复 ACK 被吸收；未匹配 ACK 暴露给未来 Dialog 路径。
- 已覆盖受控 INVITE/响应/ACK 丢包、超时、重复消息、2xx 边界和真实 Netty UDP INVITE/486/ACK 闭环。
- 3C 使用经典 RFC 3261 的 2xx 终止行为；RFC 6026 Accepted、Timer M/L 和重复 2xx 转交留在 3D。
...
- ICT/IST 已增加 RFC 6026 Accepted 状态以及 Timer M/L。
- ICT 在 Accepted 生命周期内持续向 TU 转交匹配的重复或分叉 2xx。
- IST 在 Accepted 中吸收重复 INVITE，2xx ACK 继续分流到 TU/Dialog 边界。
- `InviteCancellations` 使用原 INVITE branch、Request-URI、路由字段和 CSeq 数字生成独立 CANCEL。
- `CancelCoordinator` 创建 CANCEL NICT/NIST、使用派生 INVITE key 关联 IST，并自动返回 200 或 481。
- 原 IST 仅在 Proceeding 时向 TU 发出一次取消通知，由 TU 生成 487；已有最终响应时 CANCEL 不改变原事务。
- `SipTransactionDispatcher` 根据请求 Method 或响应 CSeq Method 统一分流 INVITE、ACK、CANCEL 和其他事务。
- 已验证容量拒绝、关闭清理、TU 回调阻塞不影响 Timer，以及真实 UDP INVITE/CANCEL/487/ACK 闭环。
...
- 首个 2xx 进入 Accepted，每个匹配的重复或分叉 2xx 都继续转交 TU。
...
- 首次 INVITE 通知 TU。
- 重复 INVITE 不重复通知 TU。
- 自动 `100 Trying` 策略留给后续 Stack policy，不与本阶段核心状态转换耦合。
- 1xx 保持 Proceeding。
- 2xx 进入 Accepted 并启动 Timer L。
- Accepted 吸收重复 INVITE；应用可以继续通过 Handle 提交 2xx 发送。
...
- 匹配 CANCEL 自动响应 200，未匹配 CANCEL 自动响应 481。
- 原 IST 仍为 Proceeding 时通知 TU 生成 `487 Request Terminated`；已有最终响应时不改变原 IST。
...
下面的组件图展示 Transport、统一路由、单个 Transaction、Timer、CANCEL 协调和 TU 之间的最终关系。
...
flowchart TB
    ...
    subgraph Routing["Transaction Routing"]
        TOP_DISPATCHER["SipTransactionDispatcher<br/>3D 已实现"]
        DISPATCHER["NonInviteTransactionManager<br/>3B 已实现"]
        INVITE_DISPATCHER["InviteTransactionManager<br/>3C 已实现"]
        CANCEL_COORDINATOR["CancelCoordinator<br/>3D 已实现"]
    ...
    subgraph Transaction["每个 Transaction 实例"]
        ...
        INVITE_STATE_MACHINE["ICT / IST<br/>3C 已实现"]
    ...
    NETTY --> HANDLER
    HANDLER --> TOP_DISPATCHER
    TOP_DISPATCHER -. "Non-INVITE / CANCEL" .-> DISPATCHER
    TOP_DISPATCHER -. "INVITE / ACK" .-> INVITE_DISPATCHER

    DISPATCHER -. "解析路由 Header" .-> HEADERS
    DISPATCHER -. "生成 TransactionKey" .-> KEY_FACTORY
    KEY_FACTORY --> HEADERS
    DISPATCHER -. "find / getOrCreate" .-> REPOSITORY
    INVITE_DISPATCHER -. "INVITE / response / ACK" .-> REPOSITORY
    CANCEL_COORDINATOR -. "CANCEL NICT / NIST" .-> DISPATCHER
    CANCEL_COORDINATOR -. "related INVITE key" .-> INVITE_DISPATCHER
...
第三阶段 3A-3D 已完成：Header/Key/Repository/Mailbox/Timer 基础设施、Non-INVITE 和 INVITE 状态机、RFC 6026 Accepted、ACK 分流、CANCEL 协调、统一 Dispatcher 以及 UDP 端到端验证均已落地。下一步进入 Dialog Layer 设计与实现，JAIN-SIP Adapter 继续在核心公共 API 稳定后单独讨论。
