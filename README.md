...
| UPDATE / Session Timer | 进行中（6E-A） | 已完成 UPDATE Early/Confirmed Dialog 与 Session-Expires、Min-SE Header 基础；协商和刷新 Timer 待实现 |
...
5. **认证与 SIP 扩展（6A～6D 已完成，6E～6G 待执行）**：请求尝试基础、UAC/UAS Digest、PRACK/100rel；后续实现 UPDATE、Session Timer 和 INFO，完整 REFER 与事件框架后置。
