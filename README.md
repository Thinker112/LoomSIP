...
| TCP/TLS Transport | 实施中（5A～5D 已完成） | TCP/TLS 流式分帧、Client/Server、TLS 握手、连接复用和统一协议选择已完成；完整资源限制待 5E |
...
4. **可靠传输（实施中，5A～5D 已完成）**：TCP/TLS 分帧、连接复用、TLS 握手、统一协议选择、失败传播和资源限制。
